...
# Usage: bash alignAndExtract_hs38DH_NoAlt.for_cram_input_by_WZ.sh <sample_id> <cram_file> <cram_reference_fasta> <kourami_ref_db> <output_path>
# Example:
...
function usage {
    ...
    echo "USAGE: <PATH-TO>/$me <sample_id> <cram_path> <cram_ref> <kourami_db> <output_path>"
    echo
...
merged_hla_panel=$kourami_db/hs38NoAltDH.fa
bam_for_kourami=${output_path}/$sampleid\_on_KouramiPanel.bam
...
if [ ! -e "$cram_path" ] || [ ! -e "$cram_ref" ] || [ ! -e "$kourami_db" ] || [ ! -e "$merged_hla_panel" ];then
...
$samtools_bin view -T ${cram_ref} -b ${cram_path} chr6:29723340-29727296 chr6:29726601-29749049 chr6:29826979-29831122 \
    ...
    HLA-DRB1*16:02:01:| $samtools_bin sort --thread $num_processors -m $samtools_sort_memory_per_thread -O BAM - > ${output_path}/$sampleid.extract.bam
...
$samtools_bin index ${output_path}/$sampleid.extract.bam

echo ">>>>>>>>>>>>>> bamUtil fastq extraction (38DH_NoAlt)"
$bamUtil bam2FastQ --in ${output_path}/$sampleid.extract.bam --gzip --firstOut ${output_path}/$sampleid\_extract_1.fq.gz --secondOut ${output_path}/$sampleid\_extract_2.fq.gz --unpairedOut ${output_path}/$sampleid\_extract.unpaired.fq.gz &> /dev/null
...
if [ ! $OUT -eq 0 ];then
    ...
    rm ${output_path}/$sampleid.extract.bam* ${output_path}/$sampleid\_extract.unpaired.fq.gz
fi

echo ">>>>>>>>>>>>>> bwa mem to hla panel for Kourami "
$bwa_bin mem -t $num_processors $merged_hla_panel ${output_path}/$sampleid\_extract_1.fq.gz ${output_path}/$sampleid\_extract_2.fq.gz | $samtools_bin view -Sb - > $bam_for_kourami
OUT=$?
if [ ! $OUT -eq 0 ];then
    echo 'bwa alignment of extracted reads to HLA panel failed...'
    exit 1
fi
