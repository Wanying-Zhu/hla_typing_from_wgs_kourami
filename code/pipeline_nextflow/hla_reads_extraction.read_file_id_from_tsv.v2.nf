...
process EXTRACT_HLA {
    ...
    // Docker image has been pushed to ICA repository
    // container '079623148045.dkr.ecr.us-east-1.amazonaws.com/cp-prod/8104b7c9-70b5-4a0d-a284-91532d5e99a3:latest'
    container '079623148045.dkr.ecr.us-east-1.amazonaws.com/cp-prod/b3b59a81-8881-4228-8a5b-38b052c2b7b4:latest'
    // Specify the CPU etc.
    cpus = 4
    memory = '16 GB'
    ...
    input:
    tuple val(sampleid),
          path(cram_file, stageAs: 'input.cram'),
          path(cram_index, stageAs: 'input.cram.crai'),
          path(cram_ref, stageAs: 'input.cram.reference')
    ...
    samtools view -T ${cram_ref} -b -X ${cram_file} ${cram_index} chr6:29723340-29727296 chr6:29726601-29749049 chr6:29826979-29831122 \
        ...
        HLA-DRB1*16:02:01:| samtools sort --thread 8 -m 2G -O BAM - > ${sampleid}.extract.bam
    echo "# Done"
    """
}
